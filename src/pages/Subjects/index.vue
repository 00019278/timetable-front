...
function copyItem(item: SubjectModel) {
                    ...
                    <template #default="{ row }">
                        <div class="flex items-center gap-3">
                            <div class="flex gap-0.5">
                                <div v-for="i in 10" :key="i" class="w-[3px] h-5 rounded-[1px] transition-all" :class="i <= (row.weight || 0) ? 'bg-amber-400' : 'bg-gray-200'"></div>
                            </div>
                            <span class="text-xs text-gray-500 font-medium w-8 text-right">{{ row.weight || 0 }}/10</span>
                        </div>
                    </template>
                </el-table-column>
